...
def run_permutation(perm):
    ...
    for i in perm:
        ...
        c.send_input(i)
        computers.append(c)
        threads.append(threading.Thread(target=thread_func, args=(c,)))
        threads[-1].start()

    input = 0
    while computers[-1].state != 'finished':
        print "  Loop"
        for c in computers:
            c.send_input(input)
            print "   ", c.state
            output = c.get_output()
            print "   ", c.state, input, output
            input = output
    return input

best = -999999999
#for perm in [(9,8,7,6,5)]: #permutations(range(5,10)):
for perm in permutations(range(5,10)):
    print "LGV", perm
    val = run_permutation(perm)
    if val > best:
        best = val
        print "new best", perm, val
