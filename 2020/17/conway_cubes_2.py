...
def do_round(cube):
    ...
    for i in range(x):
        for j in range(y):
            for k in range(z):
                for l in range(zz):
                    neighbors = cube[max(i - 1, 0):min(i + 2, x), max(j - 1, 0):min(j + 2, y), max(k - 1, 0):min(k + 2, z), max(l - 1, 0):min(l + 2, zz)]
...
start_cube = np.zeros((square_size, square_size, 13, 13), dtype=bool)
for i, row in enumerate(rows):
    for j, active in enumerate(row):
        if active == '#':
            start_cube[i + 6, j + 6, 6, 6] = True
